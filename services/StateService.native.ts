...
export class StateService {
  ...
  static async saveState(blocks: BlockData[]): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(STORAGE_KEYS.BLOCKS_STATE, JSON.stringify(state));
  ...
  static async loadState(): Promise<AppState | null> {
    try {
      const savedState = await AsyncStorage.getItem(STORAGE_KEYS.BLOCKS_STATE);
  ...
  static async clearState(): Promise<void> {
    try {
      await AsyncStorage.removeItem(STORAGE_KEYS.BLOCKS_STATE);
